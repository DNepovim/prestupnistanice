---
slug: balkovsky-pegas
title: Balkovský Pegas
date: 2026
pagesCount: 216
isbn: 978-80-909036-4-7
authors:
  - author: src/content/authors/jaromir-sebek.md
    slug: jaromir-sebek
    isMain: true
    role:
      - author
  - author: src/content/authors/anna-ticha.md
    slug: anna-ticha
    role:
      - cover
      - illustration
  - author: src/content/authors/jakub-skalak.md
    slug: jakub-skalak
    role:
      - supereditor
  - author: src/content/authors/petr-blaha.md
    slug: petr-blaha
    role:
      - reviewer
  - author: src/content/authors/aida-lihova-legnerova.md
    slug: aida-lihova-legnerova
    role:
      - reviewer
  - author: src/content/authors/karolina-dalstroem.md
    slug: karolina-dalstroem
    role:
      - translate
  - author: src/content/authors/petr-blaha.md
    slug: petr-blaha
    role:
      - corrector
  - author: src/content/authors/martina-nejedla.md
    slug: martina-nejedla
    role:
      - corrector
  - author: src/content/authors/hynek-tippelt.md
    slug: hynek-tippelt
    role:
      - corrector
  - author: src/content/authors/michaela-zoulova.md
    slug: michaela-zoulova
    role:
      - corrector
  - author: src/content/authors/filip-skalak.md
    slug: filip-skalak
    role:
      - typesetting
cover: /images/Pegas_cover.png
category: novel
color: '#003cac'
bgColor: '#87b3ef'
claim: >-
  „A přece v dávných dobách, jež se dnes již propadly v bezhlesé zapomenutí,
  byla tato ves čímsi neobyčejná. Místním venkovanům byl udělen dar – či snad
  prokletí? – nahlížet v hlubiny a brodit se temnotami země.
---

A právě tady se kdysi udál zázrak, jenž propojil nebe a zemi. O těchto bělostných křídlech, o smrti i pokušení, o daru i zatracení vypráví náš příběh…“

Balkovský Pegas Jaromíra Šebka je román, v němž se zapomenutý český venkov mění v jeviště lidských vášní, děsů i drobných zázraků. Autor s citem pro archaický jazyk a jemnou ironii rozehrává příběh, který spojuje hororové archetypální obrazy s humorem a lyrickou obrazností. Výsledkem je mnohovrstevnaté, pitoreskní vyprávění, kde se každodennost mísí s fantaskními vizemi…
